...
/** Isolate OG lookup from this product's public/og.jpg + site.json. */
function isolated(ctx = {}) {
  const { cwd, site, ...rest } = ctx;
  return {
    cwd: cwd ?? mkdtempSync(join(tmpdir(), "grok-pwa-blank-")),
    site: site ?? {},
    ...rest,
  };
}
...
// Tripwires: Next.js metadata + the grok manifest route replace Vite/Nitro PWA wiring.
test("next layout keeps PWA metadata", () => {
  ...
  assert.match(nextConfig, /__grok\/manifest\.webmanifest/);
});
