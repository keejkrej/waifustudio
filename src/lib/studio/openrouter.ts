...
function headers(apiKey: string) {
  return {
    ...
    "HTTP-Referer": "https://waifustudio.vercel.app",
    "X-Title": "WaifuStudio",
  };
}
...
export function requireKey(apiKey: unknown): string {
...
export async function jsonError(err: unknown, status = 400): Promise<Response> {
...
export async function generateImage(data: GenerateImageInput) {
  const key = requireKey(data.apiKey);
...
export async function submitVideo(data: SubmitVideoInput) {
  const key = requireKey(data.apiKey);
  const body: Record<string, unknown> = {
    model: data.model,
    prompt: data.prompt,
...
export async function pollVideo(data: PollVideoInput) {
  const key = requireKey(data.apiKey);
...
export async function fetchVideoContent(data: PollVideoInput) {
  const key = requireKey(data.apiKey);
...
export async function expandPrompt(data: ExpandPromptInput) {
  const key = requireKey(data.apiKey);
  const sys =
    data.kind === "motion"
...
export async function pingKey(apiKey: string) {
