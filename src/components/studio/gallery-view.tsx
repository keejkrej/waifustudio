...
export function GalleryView() {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 md:p-6">
      ...
      <h1 className="mt-1 text-2xl font-medium tracking-tight md:text-3xl">Gallery</h1>
      <p className="mt-1 hidden max-w-lg text-sm text-muted-foreground md:block">
        Results stay on this device. The sample still is ready to preview; API clips land in the
        local gallery.
      </p>
      {gallery.length === 0 ? (
        <p className="mt-8 text-sm text-muted-foreground">No cuts yet.</p>
      ) : (
